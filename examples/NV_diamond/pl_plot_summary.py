...
def main():
    ...
    functional_runs = ["pbe_out", "frac_pbe_out", "hse06_out", "frac_hse06_out"]

    # Order of analysis sub-pipelines matching your structural directory trees
    pipeline_modes = ["abs_gs_force_mode", "gs_zpl_disp_mode", "zpl_ems_force_mode"]
    ...
    for run in functional_runs:
        ...
        md_content.append(
            f"Automated quick-view gallery compiled for the `{run}` functional calculation pipeline.\n"
        )
        md_content.append("---")
        ...
        for pipeline in pipeline_modes:
            ...
            for i in range(0, len(primary_plots), 2):
                ...
                if path_left.exists():
                    ...
                    md_content.append(
                        f'    <td align="center" width="50%"><b>{title_left}</b><br/><img src="{rel_img_link}" width="100%"/></td>'
                    )
                else:
                    md_content.append(
                        '    <td align="center" width="50%" style="color:gray;">Plot missing</td>'
                    )
                ...
                if i + 1 < len(primary_plots):
                    ...
                    if path_right.exists():
                        ...
                        md_content.append(
                            f'    <td align="center" width="50%"><b>{title_right}</b><br/><img src="{rel_img_link}" width="100%"/></td>'
                        )
                    else:
                        md_content.append(
                            '    <td align="center" width="50%" style="color:gray;">Plot missing</td>'
