...
class VibrationalSpectra1D:
            ...
            for lx in range(l1 + 1):
                ...
                try:
                    pr2 = (
                        ...
                        / (self.factorial(k) * self.factorial(l) *
                           self.factorial(k1 - kx) * self.factorial(l1 - lx))
                        * 2.0**((k + l - m - n) // 2)
                    )
                except ValueError:
                    pr2 = 0
    ...
    def compute_overlap_matrix(self):
        for i in range(self.NN1 + 1):
            for j in range(self.NN2 + 1):
                self.overlap_matrix[i, j] = self.overlap(i, j, self.rho, self.cosfi, self.sinfi)
    ...
    def compute_spectrum(self):
        ...
        print(f'This number should be close to 1.00, if that is not the case, increase NN1 and NN2: {Rpart:.6f}')
    ...
    def compute_lineshape(self):
        ...
        for l in range(self.M):
            ...
            for k, c in enumerate(self.contr):
                deltaE = self.en[k] - E
                dos[l] += c * np.exp(-deltaE**2 / (2 * self.sigma**2)) / (self.sigma * np.sqrt(2 * self.PI))
            dosw3[l] = dos[l] * E**3
    ...
    def save_results(self, overlap_file='overlap.json', lineshape_file='lineshape.json'):
