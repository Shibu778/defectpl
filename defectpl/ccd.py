...
class ConfigurationCoordinateDiagram:
    ...
    def generate_structures(
        self, displacements: np.ndarray, remove_zero: bool = True
        ...
        if remove_zero:
            displacements = displacements[displacements != 0.0]

        ground_structs = self.ground_struct.interpolate(
            self.excited_struct, nimages=displacements
        )
        excited_structs = self.ground_struct.interpolate(
            self.excited_struct, nimages=(displacements + 1.0)
        )
    ...
    def setup_calculations(
        ...
        input_files: List[str] = ["KPOINTS", "POTCAR", "INCAR"],
    ) -> None:
        """
    ...
    def extract_PES(
        self, vasprun_paths: List[str], tol: float = 0.001
    ...
    def get_omega_from_PES(
        ...
        bounds = (
            (-np.inf, np.inf)
            if Q0 is None
            else ([-np.inf, Q0 - 1e-10, -np.inf], [np.inf, Q0 + 1e-10, np.inf])
        )
    ...
    def analyze_ccd(
        self,
        ground_vaspruns: List[str],
        excited_vaspruns: List[str],
        dE: float = 0.0,
        plot: bool = True,
        figsize: Tuple[int, int] = (5, 5),
        xlim: Tuple[float, float] = (-3, 10),
        ylim: Tuple[float, float] = (-0.5, 4),
