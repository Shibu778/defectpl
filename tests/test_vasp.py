...
def test_get_structures_and_forces_with_poscar():
    ...
    structures, forces = get_structures_and_forces(MOCK_OUTCAR_PATH, poscar_path=MOCK_POSCAR_PATH)
    assert len(structures) == 2
...
def test_get_first_and_last_standalone():
    ...
    struct_first, force_first = get_first_structure_and_forces_from_outcar(MOCK_OUTCAR_PATH)
    struct_last, force_last = get_final_structure_and_forces_from_outcar(MOCK_OUTCAR_PATH)
