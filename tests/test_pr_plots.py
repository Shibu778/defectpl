...
def _make_ks_data(
    n_up: int = 6, n_down: int = 6, vbm: float = -0.5, cbm: float = 0.5
) -> KohnShamPlotData:
    """
    ...
    def _make_eigenvals(n, e_lo=-1.5, e_hi=1.5):
        return [
            [e_lo + (e_hi - e_lo) * i / max(n - 1, 1), 1.0 if i < n // 2 else 0.0]
            for i in range(n)
        ]
    ...
    def _occ_split(xvals, energies, occs):
        occ = {
            "xvalues": [x for x, o in zip(xvals, occs) if o > 0.6],
...
class TestPlotPrVsEnergy:
    ...
    def test_threshold_line_present(self):
        ...
        hlines = [line for line in ax.lines if line.get_linestyle() == "--"]
...
class TestPlotKsWithPr:
    ...
    def test_ipr_metric(self, tmp_path):
        ...
        plot_ks_with_pr(self.ks_data, self.pr_result, metric="ipr", output_filename=out)
        assert out.exists()
        plt.close("all")
    ...
    def test_custom_cmap(self, tmp_path):
        ...
        plot_ks_with_pr(
            self.ks_data, self.pr_result, cmap="viridis", output_filename=out
        )
        assert out.exists()
        plt.close("all")
    ...
    def test_custom_vmin_vmax(self, tmp_path):
        ...
        plot_ks_with_pr(
            self.ks_data, self.pr_result, vmin=0.1, vmax=0.5, output_filename=out
        )
        assert out.exists()
        plt.close("all")
...
class TestCliPrPlotEnergy:
    ...
    def _invoke(self, args, result_dict=None):
        ...
        with tempfile.TemporaryDirectory() as td:
            ...
            rv = self.runner.invoke(
                pr_group, ["plot", json_path, "--out", out_path] + args
            )
            return rv, out_path
    ...
    def test_energy_mode_creates_file(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            rv = self.runner.invoke(
                pr_group, ["plot", json_path, "--xaxis", "energy", "--out", out_path]
            )
    ...
    def test_band_mode_exit_zero(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            with open(json_path, "w") as fh:
                json.dump(self.result, fh)
            rv = self.runner.invoke(
                pr_group, ["plot", json_path, "--xaxis", "band", "--out", out_path]
            )
            assert rv.exit_code == 0, rv.output
            assert os.path.exists(out_path)
